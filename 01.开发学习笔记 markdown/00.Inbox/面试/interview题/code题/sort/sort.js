...
function heapSort(arr) {
  ...
  // 这个循环的目的是不断将堆顶元素（最大元素）与堆中的最后一个元素进行交换，
  //然后排除最后一个元素，再通过调整堆使剩余元素重新构成一个最大堆。
